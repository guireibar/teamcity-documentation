...
TeamCity 2020.2 adds a [new responsibility](multinode-setup-for-high-availability.md#Processing+User+Requests+to+Modify+Data+on+Secondary+Node) for secondary nodes. If granted to a node, it allows UI actions: running, stopping, tagging, commenting builds, and much more. The [list of supported UI actions](multinode-setup-for-high-availability.md#User-level+Actions+on+Secondary+Node) now also includes editing projects and build configurations (with a few limitations, such as editing cloud profiles).   
