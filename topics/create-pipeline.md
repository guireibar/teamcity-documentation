...
To view the statuses of all chained builds, go to __Build Configuration Home__ of any of these builds and open the __Dependency__ tab of __Build Results__:
...
2. Open advanced options and then enable the option to _trigger a build on changes in snapshot dependencies_. This way, this trigger will also react to changes relevant to the _TodoApp_ config.  
   It's often convenient to add a single trigger at the end of the chain and just enable this option to consider the previous builds. Whenever you want to change the triggering settings, you can do this in one place.
...
### Restricting Checkout Scope

Every chain stage is responsible for its own task. And in most cases, different build configs need to monitor different parts of the source project. For example, our _TodoImage_ is mostly interested in changes made to `Dockerfile`, and it makes sense to restrict its scope to it. This way, when you change the app's logic, only _TodoApp_ will be triggered, and _TodoImage_ will run as its dependency build. Without such restrictions, TeamCity would start both of these builds per any change in the source repo.
...
The build chain mechanism in TeamCity is very flexible and designed to satisfy the needs of every project. You will also notice that build chains are much easier to monitor than scattered builds. Detailed statuses of all chained builds are displayed in the __Dependencies__ tab of __Build Results__. And if there is a composite build that depends on multiple test builds, it will report the results of all these tests in one place.
